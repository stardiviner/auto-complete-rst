...
class InfoGetterDocutils(InfoGetter):
    ...
    def get_directives_sub_modules():
        ...
        sub_module_names = list(set(
            m for (d, (m, c)) in directives._directive_registry.items()))
    ...
    def _getinfo_directives(self):
        ...
        for (dirname, (modname, clsname),
             ) in directives._directive_registry.items():
            clsobj = getattr(getattr(sub_modules, modname), clsname)
            self._add_directive(dirname, clsobj)

        for (dirname, clsobj) in directives._directives.items():
            self._add_directive(dirname, clsobj)
    ...
    def _getinfo_roles(self):
        ...
        role_registry.update(
            (a, role_registry[r]) for (a, r) in en.roles.items())

        unimplemented = roles.unimplemented_role
        self.roles = sorted(
            r for (r, f) in role_registry.items()
...
class InfoGetterSphinx(InfoGetter):
    ...
    def _getinfo_domain(self, domain):
        ...
        for (dirname, clsobj) in domain.directives.items():
            self._add_directive(genename(dirname), clsobj)
        self.roles.extend(list(map(genename, domain.roles)))

    def getinfo(self):
        list(map(self._getinfo_domain, self.domains))
    ...
    def with_buildins(cls):
        ...
        self = cls()
        list(map(self.add_domain, iter(BUILTIN_DOMAINS.values())))
        return self
    ...
    def add_extension(self, extension):
        ...
        list(map(self.add_domain, fakeapp.domains))
    ...
    def get_ext_setup_from_path(extension):
        if extension.endswith('.py'):
            ...
            namespace = {}
            exec(compile(open(extension).read(), extension, 'exec'), namespace)
...
def genelisp(extension=[]):
    ...
    list(map(igsphinx.add_extension, extension))
    igsphinx.getinfo()
